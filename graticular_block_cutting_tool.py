...
class GraticularBlockCuttingTool:
    ...
    def PassInitialChecks(self):
        ...
        if number_of_layers >= 2:
            ...
            if top_layer.geometryType() == QgsWkbTypes.LineGeometry:
    ...
    def PopPolygon_Area_Fields(self, layer):
        ...
        for feature in iterator:
            ...
            geom = feature.geometry()             # geometryAndOwnership()
    ...
    def Populate_Polygon_Part_and_Total_Fields(self):
        ...
        for i in range(1, number_of_layers, 1):
            ...
            for each_new_feature in new_features:
                ...
                if BLOCK_ID in feature_dict:
                    feature_dict[BLOCK_ID]['Total'] = feature_dict[BLOCK_ID]['Total'] + 1
                else:
                    feature_dict[BLOCK_ID] = {}
                    feature_dict[BLOCK_ID]['Total'] = 1

                feature_dict[BLOCK_ID]['Part'] = 0

            self.progressBar_Init(len(feature_dict))
            self.Add_Message("Need to prcess %d blocks %s" % (len(feature_dict), datetime.datetime.now()))
            i = 0
            ...
            for each_new_feature in new_features:
                ...
                feature_dict[each_BLOCK_ID]['Part'] = feature_dict[each_BLOCK_ID]['Part'] + 1

                if field_index_Part != -1:
                    layer.changeAttributeValue(each_new_feature.id(), field_index_Part, str(feature_dict[each_BLOCK_ID]['Part']))

                if field_index_Total != -1:
                    layer.changeAttributeValue(each_new_feature.id(), field_index_Total, str(feature_dict[each_BLOCK_ID]['Total']))
    ...
    def EditUserSettings(self):
        ...
        for setting in root.findall("./userSettings/GraticularBlockCuttingTool.My.MySettings/setting"):
            ...
            setting_name = QTableWidgetItem(setting.get('name'))
            setting_name.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)    # make column NOT editable !! http://stackoverflow.com/questions/17104413/pyqt4-how-to-select-table-rows-and-disable-editing-cells
            setting_value = QTableWidgetItem(setting.find('value').text)
